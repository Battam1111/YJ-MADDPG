...
import torch
from env.utils import *       # 包含各种工具函数，如 addSphere, caculate_2D_distance, direction_normalize, velocilty_normalize 等
from env.scene import *       # 场景构造模块，负责加载环境中各类物体
...
class Drone(object):
    ...
    def __init__(self, basePos: list = [0., 0., 0.], sence_loadItems: dict = None, signalPointId2data: dict = None, 
        ...
        for file in os.listdir("hcanet-3.27_maddpg_old/config"):
            path = os.path.join("hcanet-3.27_maddpg_old/config", file)
    ...
    def greedy_action(self):
        ...
        for SP, data in self.signalPointId2data.items():
            ...
            diss = caculate_2D_distance(currentPos, pos_SP)
            if diss <= self.LASER_LENGTH:
                if diss<min_dis and data > 0:
                    min_dis=diss
                    min_pos=np.array(pos_SP)
        if min_dis!=100000:
            direction = torch.tensor(min_pos[:2] - np.array(currentPos)[:2]).to(self.device)
            direction = direction / torch.sqrt((direction**2).sum())
            actions = direction
        else:
            actions =  torch.tensor([random.uniform(-1,1) for _ in range(2)]).to(self.device)
            actions = actions / torch.sqrt((actions**2).sum())
            actions = actions
        return actions
...
class ChargeUAV(object):
    ...
    def greedy_action(self, UAV_energy, robot_pos):
        currentPos, _ = p.getBasePositionAndOrientation(self.robot)
        min_energy = 10000
        min_index = -1
        for i,energy in enumerate(UAV_energy):
            if min_energy > energy[0]:
                min_energy = energy[0]
                min_index = i
        if  min_index != -1:
            direction = torch.tensor(robot_pos[min_index][:2] - np.array(currentPos)[:2]).to(self.device)
            direction = direction / torch.sqrt((direction**2).sum())
            actions = direction
        else:
            actions =  [random.uniform(-1,1) for _ in range(2)]
            actions = actions / torch.sqrt((actions**2).sum())
...
if __name__ == "__main__":
    ...
    # 请确保配置文件 hcanet-3.27_maddpg_old/config 中有正确的参数设置

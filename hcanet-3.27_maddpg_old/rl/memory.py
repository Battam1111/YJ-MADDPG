...
class ReplayMemory:
    def __init__(
        ...
        self.buf_state = torch.empty((self.max_len, n_agents, state_dim), device=self.device)
        # self.buf_state = []
    ...
    def convert_experience_to_stateAndOther(self, state, action, next_state, reward, done):
    ...
    # 原memory接口
    def push(self, state, action, next_state, reward, done, i_episode):
        state, other = self.convert_experience_to_stateAndOther(state, action, next_state, reward, done)

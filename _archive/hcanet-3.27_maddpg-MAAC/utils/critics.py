...
class AttentionCritic(nn.Module):
    ...
    def scale_shared_grads(self):
        ...
        for p in self.shared_parameters():
            if p.grad is not None:  # 检查是否为 None
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            for i, a_i, selector in zip(range(len(agents)), agents, curr_head_selectors):
                ...
                if keys:
                    ...
                    # 如果 keys 为空，直接填充0
                    other_all_values[i].append(torch.zeros_like(selector))
                    all_attend_logits[i].append(torch.zeros(selector.shape[0], 1, 1))
                    all_attend_probs[i].append(torch.ones(selector.shape[0], 1, 1))
                    
        # calculate Q per agent
        all_rets = []
        for i, a_i in enumerate(agents):
            head_entropies = [(-((probs + 1e-8).log() * probs).squeeze().sum(dim=-1 if probs.dim() > 1 else 0).mean()) for probs in all_attend_probs[i]]

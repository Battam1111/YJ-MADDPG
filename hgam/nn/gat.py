...
from hgam.nn._isin_util import isin
   ...
   def forward(self, x, edge_index, node_type):
      ...
      for i in range(len(self.layers)):
         ...
         if self.full_receptive_field or i == len(self.layers) - 1:
            final_x.append(x[agent_nodes])#what is agent_nodes
...
class GATModule(torch.nn.Module):
   ...
   def __init__(self,
      ...
      for i in range(len(self._sizes) - 1):
         ...
         self.layers.append(GATConv(in_size, out_size, n_heads, concat=concat, dropout=dropout, add_self_loops=add_self_loops))

      def init_kaiming(m):
         if type(m) == torch_geometric.nn.GATConv:
...
class RGCNModule(GraphModule):
   ...
   def _generate_edge_types(self, edge_index: torch.tensor, node_type: torch.tensor) -> torch.tensor:

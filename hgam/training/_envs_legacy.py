...
class GraphEnv:
   ...
   def add_all_agent_edges(self, state: Data, agent_types: torch.tensor,
                           node_types: torch.tensor) -> Data:
      ...
      agent_edge_index = torch.tensor([agent_edge_source, agent_edge_target], dtype=th.long)
...
class SMACEnv(GraphEnv, StarCraft2Env):
   ...
   def _visibility_matrix_to_edge_index(adj: np.ndarray):
      ...
      :rtype: torch.tensor
      """
      coo = torch.zeros((2, np.sum(adj > 0)), dtype=th.long)
   ...
   def get_graph_state(self,
      ...
      if not v2:
         ...
         x = torch.tensor(self.get_obs())
         if include_unit_ids:
            x = torch.cat((th.eye(x.size(0)), x), dim=1)
